...
> Proto3，即Protocol Buffers， 是一种与语言无关，平台无关的可扩展机制，用于序列化结构化数据。使用Protocol Buffers 可以一次定义结构化的数据，然后可以使用特殊生成的源代码轻松地在各种数据流中使用各种语言编写和读取结构化数据。
>
...
> 消息messge是用来定义数据的，服务service是用来定义发送请求的方法的。
>
> ```protobuf
> ...
...
>     string exposure=1;
>     int64 time_stamp=2;
>     repeated Article recommends=3;  // repeated 重复出现的 对应的是列表
> }
> 
...
>
> - [关于枚举，只写python小伙伴都表示一脸懵逼，点击了解更多](https://www.cnblogs.com/traditional/p/11111339.html)
...
> 消息字段可以是以下之一，表示字段的值是否为单一的值：
>
> - singular：格式良好的消息可以包含该字段中的零个或一个（但不超过一个）。
>
> - repeated：对应Python的列表，将保留重复值的顺序。
>
>   ```protobuf
>   message Result {
>     string url = 1;
