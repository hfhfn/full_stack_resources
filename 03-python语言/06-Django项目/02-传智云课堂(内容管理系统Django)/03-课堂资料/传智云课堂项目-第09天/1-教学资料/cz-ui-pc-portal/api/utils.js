...
export default {
  getActiveUser:function(){
    let uid = this.getCookie("juid")
    if(uid){
      let activeUserStr = this.getSession("activeUser");
      if (!activeUserStr) {
        let activeUser = this.getUserInfoFromJwt(uid)
        console.log(activeUser);
        if (activeUser) {
          let activeUserStr = JSON.stringify(activeUser)
          this.setSession("activeUser", activeUserStr);
        }
      }
  ...
  getUserInfoFromJwt : function (jwt) {
    ...
    activeUser.phone = jwtDecodeVal.phone || '';
